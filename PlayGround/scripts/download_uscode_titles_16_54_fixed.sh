...
# 🇺🇸 USA Code Download Script - Titles 16-54 (2023 Edition) - FIXED VERSION
# Downloads remaining USA Code titles with proper error handling
# Fixed curl command issues from previous version
...
LOG_FILE="download_log_titles_16_54_fixed_$(date '+%Y%m%d_%H%M%S').txt"

echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     USA Code Download - Titles 16-54 (FIXED VERSION)       ║${NC}"
echo -e "${BLUE}║               2023 Edition with Proper Error Handling        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
...
# Function to download file with improved error handling
download_with_retry() {
    local url="$1"
    local output_path="$2"
    local description="$3"
    ...
    while [ $retry_count -lt $MAX_RETRIES ]; do
        ...
        # Use curl with better error handling - removed --fail flag
        if curl -L --silent --show-error --connect-timeout 30 --max-time 300 "$url" -o "$output_path" 2>/dev/null; then
...
url_exists() {
    local url="$1"
    curl --head --silent --connect-timeout 10 --max-time 30 "$url" >/dev/null 2>&1
}
...
download_chapters() {
    ...
    # Try common chapter numbers (1-100 for comprehensive coverage)
    for chapter in $(seq 1 100); do
        ...
        for suffix in A B C D; do
            ...
            if url_exists "$chapter_url_letter"; then
                ((chapters_found++))
                ...
                if url_exists "$html_url_letter"; then
...
process_title() {
    ...
    # Download main title files with priority on the most important ones
    local title_downloads=0
    
    # 1. Main title PDF (highest priority)
    ...
    # 3. Table of Contents (important for navigation)
    ...
    if url_exists "$toc_pdf_url"; then
    ...
    if url_exists "$toc_html_url"; then
        download_with_retry "$toc_html_url" "$toc_html_file" "TOC HTML"
    fi
    
    # 4. Front Matter (nice to have)
    ...
    if url_exists "$front_pdf_url"; then
    ...
    if url_exists "$front_html_url"; then
...
echo -e "${PURPLE}🔧 Improved error handling and timeout settings${NC}\n"

# Change to the correct directory and ensure download directory exists
cd "/Users/ivi/VIBE/RATIOS/iulex-Dash/iulex-v1/USA_Laws"
mkdir -p "$DOWNLOAD_DIR"

log_message "Starting USA Code download - Titles $START_TITLE-$END_TITLE"

# Process each title with progress tracking
total_titles=$((END_TITLE - START_TITLE + 1))
current_title=0

for title_num in $(seq $START_TITLE $END_TITLE); do
    ((current_title++))
    echo -e "${BLUE}📈 Progress: $current_title/$total_titles titles processed${NC}"
    ...
    # Progress checkpoint every 10 titles
    if [ $((current_title % 10)) -eq 0 ]; then
        echo -e "\n${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${PURPLE}║                    PROGRESS CHECKPOINT                       ║${NC}"
        echo -e "${PURPLE}╚══════════════════════════════════════════════════════════════╝${NC}"
        echo -e "${GREEN}✅ Titles completed: $SUCCESSFUL_DOWNLOADS${NC}"
        echo -e "${RED}❌ Titles failed: $FAILED_DOWNLOADS${NC}"
        echo -e "${BLUE}📚 Chapters downloaded: $CHAPTERS_DOWNLOADED${NC}"
        echo -e "${CYAN}📊 Progress: $(( current_title * 100 / total_titles ))%${NC}"
        echo -e "${PURPLE}═══════════════════════════════════════════════════════════════${NC}\n"
    fi
done
...
echo -e "${BLUE}📊 Total titles processed: $total_titles${NC}"

if [ $SUCCESSFUL_DOWNLOADS -gt 0 ]; then
    completion_rate=$(( SUCCESSFUL_DOWNLOADS * 100 / total_titles ))
    echo -e "${CYAN}📈 Success rate: ${completion_rate}%${NC}"
...
if [ $SUCCESSFUL_DOWNLOADS -gt 0 ]; then
    ...
    echo -e "${YELLOW}📋 Ready for Phase 2: Database integration and knowledge graphs${NC}"
else
    echo -e "\n${RED}⚠️  No successful downloads completed${NC}"
    echo -e "${YELLOW}Please check the log file: $LOG_FILE${NC}"
    echo -e "${YELLOW}Consider checking network connectivity and server availability${NC}"
fi

log_message "Download process completed. Success rate: $(( SUCCESSFUL_DOWNLOADS * 100 / total_titles ))%"
